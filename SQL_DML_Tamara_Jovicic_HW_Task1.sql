...
values 
	(current_date, (select inventory_id from inventory where film_id = (select film_id from film where title = 'Twilight' limit 1) limit 1),
	(select customer_id from customer where first_name = 'Tamara' and last_name = 'Jovicic'), current_date + interval '7 days', 1),
	(current_date, (select inventory_id from inventory where film_id = (select film_id from film where title = 'Red Notice' limit 1) limit 1),
	(select customer_id from customer where first_name = 'Tamara' and last_name = 'Jovicic'), current_date + interval '7 days', 1),
	(current_date, (select inventory_id from inventory where film_id = (select film_id from film where title = 'Oppenheimer' limit 1) limit 1),
	(select customer_id from customer where first_name = 'Tamara' and last_name = 'Jovicic'), current_date + interval '7 days', 1)
returning rental_id;
--add payment for three films
insert into payment (customer_id, staff_id, rental_id, amount, payment_date)
select r.customer_id, 1, r.rental_id, f.rental_rate, current_date
from rental r
join inventory i on r.inventory_id = i.inventory_id
join film f on f.film_id = i.film_id
where r.customer_id = (select customer_id from customer where first_name = 'Tamara' and last_name = 'Jovicic')
